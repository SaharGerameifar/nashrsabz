...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        admins = User.objects.filter(is_superuser=1, is_active=1, is_staff=1)
        admins_email = []
        user_email = self.comment.user.email
        for admin in admins:
            admins_email.append(admin.email)
        # if user_email in admins_email:
        #     user_email = False
        # print(current_site)
        if user_email:
            # email = EmailMessage(, , to=admins_email)
            EmailService.send_email(
                'ارسال ایمیل تستی',
                admins_email,
                'Emails/show_comment.html',
                {
                    'title': f'ثبت نظر برای محصول{product.title}',
                    'description': f'برای مشاهده کلیک کنید.',
